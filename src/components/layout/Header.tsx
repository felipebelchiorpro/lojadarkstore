...
export default function Header() {
  ...
  useEffect(() => {
    // Ensure cart count is updated on client after hydration
    setCartItemCount(getCartItemCount());
  }, [getCartItemCount,isAuthenticated]); // Re-check on auth change too
                ...
                <nav className="flex flex-col space-y-3">
                  ...
                  <Link href="/cart" passHref>
                    <Button variant="ghost" className="w-full justify-start text-foreground" onClick={closeSheet}>
                      <ShoppingCart className="mr-2 h-4 w-4" /> Carrinho ({cartItemCount})
                    </Button>
                  </Link>
                  {isAuthenticated ? (
                    <>
