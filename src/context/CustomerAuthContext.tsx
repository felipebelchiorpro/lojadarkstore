  ...
  const customerRegister = async (data: { name: string; email: string; password: string; phone: string }) => {
    ...
    const { data: authData, error: authError } = await supabase.auth.signUp({
      ...
      options: {
        data: {
          full_name: name,
          phone: phone, // Store phone in metadata for simple access
        },
      },
    });
    ...
    if (authError) {
      ...
      throw authError; // Re-throw to let component handle it (e.g. stop loading state)
    }
    ...
    // Auto login is handled by onAuthStateChange listener usually, but if email confirmation is required, it might not login immediately.
    // For this context, assuming email confirmation is OFF or optional for dev.
